...
public class CoordinateSystem3i implements Serializable {
     ...
     */
    public Vector3i getRelativePos(Vector3i pos) {
        return Vector3i.subtract(pos, min);
    }
     ...
     * @return - новая СК
     */
    public CoordinateSystem3i scale(int s, Vector3i center) {
        ...
        Vector3i newSize = Vector3i.mul(getSize(), s);
        ...
        int newXMin = center.x - newSize.x / (k.x + 1);
        int newYMin = center.y - newSize.y / (k.y + 1);
        int newZMin = center.z - newSize.z / (k.z + 1);

        int newXMax = center.x + newSize.x * k.x / (k.x + 1);
        int newYMax = center.y + newSize.y * k.y / (k.y + 1);
        int newZMax = center.z + newSize.z * k.z / (k.z + 1);

        return new CoordinateSystem3i(newXMin, newXMax, newYMin, newYMax, newZMin, newZMax);
    }
     ...
     * @return - новая СК
     */
    public CoordinateSystem3i translate(Vector3i offset) {
        // рассчитываем новые границы
        int newXMin = getMin().x - offset.x;
        int newYMin = getMin().y - offset.y;
        int newZMin = getMin().z - offset.z;

        int newXMax = getMax().x - offset.x;
        int newYMax = getMax().y - offset.y;
        int newZMax = getMax().z - offset.z;
