package center.buran.jmath.coordinateSystem;
...
import center.buran.jmath.vector.*;
import org.apache.commons.math3.util.Precision;

import java.io.Serializable;
import java.util.Objects;

import static center.buran.jmath.vector.Vector4d.VECTOR_4D_OPACITY;
...
public class CoordinateSystem4d implements Serializable {
     ...
     * @return - новая СК
     */
    public CoordinateSystem4d scale(double s, Vector4d center) {
        ...
        Vector4d newSize = Vector4d.mul(getSize(), s);

        // получаем коэффициенты масштабирования
        Vector4d k = new Vector4d(
                (max.x - center.x) / (center.x - min.x),
                (max.y - center.y) / (center.y - min.y),
                (max.z - center.z) / (center.z - min.z),
                (max.w - center.w) / (center.w - min.w)
        );

        // рассчитываем новые границы
        double newXMin = center.x - newSize.x / (k.x + 1);
        double newYMin = center.y - newSize.y / (k.y + 1);
        double newZMin = center.z - newSize.z / (k.z + 1);
        double newWMin = center.w - newSize.w / (k.w + 1);

        double newXMax = center.x + newSize.x * k.x / (k.x + 1);
        double newYMax = center.y + newSize.y * k.y / (k.y + 1);
        double newZMax = center.z + newSize.z * k.z / (k.z + 1);
        double newWMax = center.w + newSize.w * k.w / (k.w + 1);

        return new CoordinateSystem4d(newXMin, newXMax, newYMin, newYMax, newZMin, newZMax, newWMin, newWMax);
    }
     ...
     * @return - новая СК
     */
    public CoordinateSystem4d translate(Vector4d offset) {
        // рассчитываем новые границы
        double newXMin = getMin().x - offset.x;
        double newYMin = getMin().y - offset.y;
        double newZMin = getMin().z - offset.z;
        double newWMin = getMin().w - offset.w;

        double newXMax = getMax().x - offset.x;
        double newYMax = getMax().y - offset.y;
        double newZMax = getMax().z - offset.z;
        double newWMax = getMax().w - offset.w;
     ...
     */
    public Vector4d getRelativePos(Vector4d pos) {
        return Vector4d.subtract(pos, min);
    }
